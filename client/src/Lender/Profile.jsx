...
const Profile = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (password) formData.append('password', password); // Only update if provided
    formData.append('mobileNumber', mobileNumber);
    formData.append('address', address);
    if (image) formData.append('image', image);

    try {
      const response = await AxiosInstance.put(`/lender/update/${lenderId}`, formData);
      ...
      if (response.status === 200 && response.data.success) {
        ...
        const updatedLender = {
          ...
          mobileNumber,
          address,
          image: image ? image.name : JSON.parse(lenderData).image,
        };
        localStorage.setItem('lender', JSON.stringify(updatedLender));
      } else {
